...
mkdir -p /usr/local/share/wsgi/${app_name}
mv ${mv_flags} ${tmp_dir}/*.service /etc/systemd/system/

# prepare instance
echo "prepare instance $instance_name"
mkdir -p /usr/local/etc/wsgi/${instance_name}
chgrp -R www-data /usr/local/etc/wsgi/${instance_name}
chmod -R g+r,o-rwx /usr/local/etc/wsgi/${instance_name}
...
cd /usr/local/share/wsgi/${app_name} && sudo python3 -m venv venv
